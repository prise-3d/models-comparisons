...
def generate_data_model(_scenes_list, _filename, _interval, _choice, _feature, _scenes, _nb_zones = 4, _percent = 1, _random=0, _step=1, _each=1, _custom = False, _save_learn=None, _load_learn=None):
    ...
    # check load or not of thresholds
    save_learn_file = None
    ...
    for folder_scene in _scenes_list:
        ...
        # write into file
        # save learn indices data
        if save_learn_file:

            save_learn_file.write(folder_scene + ';')
            for i in learned_zones_indices:
                save_learn_file.write(str(i) + ';')

            save_learn_file.write('\n')

        elif load_learn_data:
            print(load_learn_data[folder_scene])
...
def main():
    ...
    parser.add_argument('--random', type=int, help='Data will be randomly filled or not', choices=[0, 1], default=1)
    ...
    parser.add_argument('--save_learn', type=str, help='Save learned zone into specific file')
    parser.add_argument('--load_learn', type=str, help='use specific learned zone from file')
    ...
    p_save_learn = args.save_learn
    p_load_learn = args.load_learn
    ...
    generate_data_model(scenes_list, p_filename, p_interval, p_kind, p_feature, scenes_selected, p_nb_zones, p_percent, p_random, p_step, p_each, p_custom, p_save_learn, p_load_learn)
